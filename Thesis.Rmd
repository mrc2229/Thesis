...
```{r}
mult_linear_1 = lm(drinker ~ . , data = imputed_data_1)

interact_check = stepAIC(mult_linear,
...
```{r propensity score}
pscore = function(x){
match.data<-matchit(drinker ~  age + bmi + gender + race + martstat, data = x, 
                 method = "full", 
