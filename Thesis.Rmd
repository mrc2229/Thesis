...
imputed_data_all = complete(imputed_data, action = "long", include = FALSE) %>%
  select(-.imp,-.id)
#complete with all 5 imputed sets
```

```{r propensity score}
prop_score = matchit(drinker ~  age + bmi + gender + race + martstat, data = imputed_data_all, 
                 method = "full", 
...
Unsure on in method should be full or nearest. Have code for both for ease.

```{r}
prop_score = matchit(drinker ~ gender + age + race + martstat + bmi, data = imputed_data_all, 
                 method = "nearest", 
                replace=TRUE, discard = "both",
                exact=c("gender", "martstat", "race"),
                 mahvars = c("bmi", "age"), 
...
#Grab matched data - will be done once prop_score is figured out
```{r, matching}
