...
```{r}
mult_linear_2 = lm(binge_drinker ~ age + bmi + gender + race + martstat, data = imputed_data_1)

interact_check = stepAIC(mult_linear_2, scope=list(upper= ~age*bmi*gender*race*martstat, lower= ~1),
        direction = c("both"))
```

* looks like best model is binge_drinker ~ age + gender + race + martstat + bmi:martstat according to stepAIC
...
```{r}
mult_linear_4 = lm(apnea ~ age + bmi + gender + race + martstat + binge_drinker, data = imputed_data_1)
...
outcome_lm_1 = glm(apnea ~ race + martstat + race:martstat + drinker, family = binomial(logit), data=z, weights= y)
}
...
outcome_lm_1 = glm(apnea ~ race + martstat + race:martstat + drinker, family = quasibinomial(), data=imputed_data_1, weights= prop_score[[1]])

summary(imputed_data_1 $ apnea)
summary(MIcombine(outcome_lm_1))
...
outcome_fun_1 = function(y) {
outcome_lm_1 = glm(apnea ~ bmi + race + martstat + binge_drinker, family = quasibinomial(), data=z, weights= y)
}
...
outcome_all_binge = lapply(imputed_data_all, outcome_fun_binge)

#summary(MIcombine(outcome_all_binge))
```


# Also ask after this how to interpret these results, confused as to what this all says about drinking and apnea's relationship. 

# Restrict binge drinking to only drinkers.
# Make everything binary and rerun. 
